...
radius = 100
nmax = 6
@time ra_idw = interp(X, Y, target; wfun=weight_idw!, radius, nmax, m=2)
@time ra_adw = interp(X, Y, target; wfun=weight_adw!, radius, cdd=25, nmax, do_angle=true, m=6)
...
for (k, m) in enumerate([1, 2, 4, 6])
  i, j = k2ij(k; nrow=2)
  @time ra_adw = interp(X, Y, target; wfun=weight_adw!, radius, cdd=25, nmax=8, do_angle=true, m)
